...
-- 7) emoji / RP-форматы / превью ---------------------------------------
T("emoji smile", GRMRPChat.Sanitize("привет :)", 512) == "привет ☺")
T("emoji heart", GRMRPChat.Sanitize("<3", 512) == "♥")
T("raw emoji preserved", GRMRPChat.Sanitize("a 🎧 b", 512) == "a 🎧 b")
local ch7, b7, x7 = GRMRPChat.ParseSay("/do огонь потихоньку", "ic")
T("/do → me via alias", ch7 == "me" and x7 and x7.cmd == "do")
ch7 = GRMRPChat.ParseSay("/try поднять ящик", "ic")
T("/try alias", ch7 == "me")
ch7 = GRMRPChat.ParseSay("/roll", "ic")
T("/roll empty allowed", ch7 == "dice")
T("RollSpec default", GRMRPChat.RollSpec("") == 100)
T("RollSpec parse d20", GRMRPChat.RollSpec("d20") == 20)
T("RollSpec clamp hi", GRMRPChat.RollSpec("99999") == 10000)
T("RollSpec clamp lo", GRMRPChat.RollSpec("1") == 2)
T("RP me fmt", GRMRPChat.RP.me.fmt("Вася", "идет") == "* Вася идет")
T("RP do self", GRMRPChat.RP["do"].fmt("Вася", "дым", { self = true }) == "* дым (возле вас)")
T("RP do other", GRMRPChat.RP["do"].fmt("Вася", "дым", nil) == "* дым (возле Вася)")
T("RP it reply-to", GRMRPChat.RP.it.fmt("Вася", "кивает", { to = "Петя" }) == "* кивает (в ответ Петя)")
T("preview me", GRMRPChat.PreviewText("Вася", "/me идет", "ic") == "* Вася идет")
T("preview plain", GRMRPChat.PreviewText("Вася", "привет", "ic") == "Вася: привет")
T("preview unknown cmd", GRMRPChat.PreviewText("Вася", "/nope x", "ic"):find("неизвестная") ~= nil)
T("preview pm", GRMRPChat.PreviewText("Вася", "/pm Петя hi", "ic") == "[ЛС] Петя: hi")
