...
if SERVER then
	...
	-- [GRM] Загрузка сохранённых экранов идёт через общий планировщик старта
	-- (GRM.Boot, тир idle): иначе десятки энтити создаются одним куском и
	-- дают микрофриз на старте карты. Без GRM работает как в оригинале.
	if GRM and GRM.Boot and GRM.Boot.OnMapStart then
		GRM.Boot.OnMapStart("textscreens.load", "idle", SpawnPermaTextscreens)
	else
