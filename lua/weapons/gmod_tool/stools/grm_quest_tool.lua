...
if CLIENT then
 ...
 local QT_FACE=Angle(0,0,90);local QT_P=Vector(0,0,0);local QT_ZONE=Color(242,190,75);local QT_ZTITLE=Color(255,220,110);local QT_BG=Color(10,16,25,235)
 local function qtp(v,dz)QT_P.x=v.x;QT_P.y=v.y;QT_P.z=v.z+dz;return QT_P end -- скретч: потребление немедленное, в цикле подряд (§6.1.8)
 hook.Add("PostDrawTranslucentRenderables","GRM_QuestTool_Preview",function(depth,sky,sky3d)
  ...
  local cv=GetConVar("grm_quest_tool_quest_id");local wanted=cv and cv:GetString()or"";if preview.id~=wanted or CurTime()>=preview.nextBuild then rebuildPreview(wanted)end
  QT_FACE.y=EyeAngles().y-90;local face=QT_FACE
  for _,z in ipairs(preview.zones)do render.DrawWireframeBox(z.c,angle_zero,z.mn,z.mx,QT_ZONE,true);cam.Start3D2D(qtp(z.c,12),face,.08);draw.SimpleText(z.title,"DermaDefaultBold",0,0,QT_ZTITLE,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER);cam.End3D2D()end
  for _,n in ipairs(preview.nodes)do render.DrawWireframeSphere(n.p,n.radius,8,8,n.col,true);render.DrawLine(n.p,n.p+n.a:Forward()*90,color_white,true);render.Model({model="models/dav0r/camera.mdl",pos=n.p,angle=n.a,scale=n.scale});if n.linked then render.DrawLine(n.p,n.linked,n.linkCol,true)end;cam.Start3D2D(qtp(n.p,18),face,.06);draw.RoundedBox(5,-150,-18,300,36,QT_BG);draw.SimpleText(n.label,"DermaDefaultBold",0,0,n.col,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER);cam.End3D2D()end
 end)
end
