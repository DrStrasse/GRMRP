...
-- Вечер-14: чат — единая библиотека lua/grm_chat (модули режима —
-- форвардеры). Enabled()/Net объявлены в sh_core библиотеки; здесь только
-- ранний стол и имена net для прочих потребителей режима (значение
-- совпадает; библиотека при загрузке сверится с ним).
